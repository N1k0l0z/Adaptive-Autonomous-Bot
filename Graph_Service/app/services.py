...
def fetch_conversation_history(
    conv_id: str, limit: int = 20
) -> List[Dict[str, Any]]:
    if not conv_id or not DATABASE_SERVICE_URL:
        return []

    try:
        url = f"{DATABASE_SERVICE_URL.rstrip('/')}/history/{conv_id}?limit={limit}"
...
def perform_vector_search(
    query: str, top_k: int = 5, min_sim: float = 0.5
) -> List[Dict[str, Any]]:
    try:
        ...
        print(f"Error executing vector search for query '{query}': {e}")
        return []